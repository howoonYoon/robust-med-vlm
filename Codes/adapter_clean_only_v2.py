...
class VLMAdapterWrapper(nn.Module):
    ...
    def extract_features(self, outputs, attention_mask: Optional[torch.Tensor] = None, debug: bool = False) -> torch.Tensor:
        ...
        # 1) 중간 레이어(-4) + first token
        hs = get(outputs, "hidden_states")
        if hs is None and debug:
            print("[debug] hidden_states is None; outputs keys:", outputs.keys() if isinstance(outputs, dict) else type(outputs))

        if isinstance(hs, (list, tuple)) and len(hs) >= 5 and torch.is_tensor(hs[-4]) and hs[-4].dim() == 3 and hs[-4].shape[-1] == H:
            return hs[-4][:, 0, :]

        # 2) fallback: last_hidden_state first token
        t = get(outputs, "last_hidden_state")
        if torch.is_tensor(t) and t.dim() == 3 and t.shape[-1] == H:
            return t[:, 0, :]

        # 3) fallback: (B,H) already pooled
        if torch.is_tensor(t) and t.dim() == 2 and t.shape[-1] == H:
            return t

        raise ValueError("No usable hidden representation found for (middle layer / first token).")
